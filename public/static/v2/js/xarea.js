...
function initialize(){
    ...
    $('.editor-store').hide();
    if(order_type > 0){
        $("input[name='city_more']").hide();
        $("#store-row-main").show();
        $('#time-row-main').hide();

    }else{
        $("input[name='city_more']").show();
        $('#time-row-main').show();
        $("#store-row-main").hide();
    }
}
...
function showStoreAll(){
    ...
    $('.show-store-shop .store-item').each(function(){
        ...
        $(this).find('.radioHidden').prop("checked",false);
    });
...
function renderingStoreShop(data){
    ...
    for (var i=0;i<data.length;i++){
        ...
        if(data[i]['shop_type'] == 1){
            html = html.replace(/\{store_icon}/g, '/static/v2/img/family.jpg');
        }else if(data[i]['shop_type'] == 2){
            html = html.replace(/\{store_icon}/g, '/static/v2/img/ok.jpg');
        }else if(data[i]['shop_type'] == 3){
            html = html.replace(/\{store_icon}/g, '/static/v2/img/hilife.jpg');
        }else{
            html = html.replace(/\{store_icon}/g, '/static/v2/img/711.png');
        }
    }
